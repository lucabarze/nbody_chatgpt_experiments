...
const std::array<Body, 5> solar_system = {{
...
void advance() {
    for (size_t i = 0; i < solar_system.size(); ++i) {
        for (size_t j = i + 1; j < solar_system.size(); ++j) {
            ...
            const_cast<Body &>(solar_system[i]).vx -= dx * solar_system[j].mass * mag;
            const_cast<Body &>(solar_system[i]).vy -=             dy * solar_system[j].mass * mag;
            const_cast<Body &>(solar_system[i]).vz -= dz * solar_system[j].mass * mag;
            const_cast<Body &>(solar_system[j]).vx += dx * solar_system[i].mass * mag;
            const_cast<Body &>(solar_system[j]).vy += dy * solar_system[i].mass * mag;
            const_cast<Body &>(solar_system[j]).vz += dz * solar_system[i].mass * mag;
        }
    }

    for (Body &body : const_cast<std::array<Body, 5>&>(solar_system)) {
        body.x += dt * body.vx;
        body.y += dt * body.vy;
        body.z += dt * body.vz;
    }
...
void offset_momentum() {
    ...
    const_cast<Body &>(solar_system[0]).vx = -px / solar_mass;
    const_cast<Body &>(solar_system[0]).vy = -py / solar_mass;
    const_cast<Body &>(solar_system[0]).vz = -pz / solar_mass;
}
